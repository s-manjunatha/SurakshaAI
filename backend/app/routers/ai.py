...
from app.models import User, FIR, ChatSession, ChatMessage, PoliceStation
...
async def chat(
    ...
    user: User = Depends(require_permission("use:ai"))
):
    session = None
    ...
    for s in similar:
        sources.append(
            AISource(
                type="fir",
                id=s["fir_id"],
                title=s["metadata"].get("fir_number", s["fir_id"]),
    ...
    # ---------------- AI RESPONSE ----------------
    ai_response = await groq_service.crime_assistant_response(
        request.message,
        context,
        history
    )
    ...
    return ChatResponse(
        ...
        sources=sources,
        actions=actions,
        structured_data=ai_response.get("structured_insights"),
        sql_query=sql_result.get("sql"),
    )
...
async def similar_cases(
    ...
    results = []
    for s in similar:
        fir_result = await db.execute(
            select(FIR).where(FIR.id == s["fir_id"])
        )
        ...
        if fir:
            results.append(
                SimilarCase(
                    ...
                    title=fir.title,
                    crime_type=fir.crime_type.value,
...
async def summarize_fir(
    ...
    result = await db.execute(
        select(FIR, PoliceStation.district)
        .join(PoliceStation)
        .where(FIR.id == fir_id)
    )
